...
contract YakRouter is Ownable {
    ...
    function findBestPathWithGas(
        ...
        queries = _findBestPathWithGas(_amountIn, _tokenIn, _tokenOut, _maxSteps, queries, tknOutPriceNwei);
    ...
    function _swapNoSplit(
        ...
        for (uint256 i = 0; i < _trade.adapters.length; i++) {
            ...
            IAdapter(_trade.adapters[i]).swap(amounts[i], amounts[i + 1], _trade.path[i], _trade.path[i + 1], targetAddress);
        }
